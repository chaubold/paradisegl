...
vec4 disdis = texture2D(dudvmap, vec2(waterTex2 * tscale));
vec4 dist = texture2D(dudvmap, vec2(waterTex1 + disdis*sca2));
vec4 fdist = dist;
...
//load normalmap
vec4 nmap = texture2D(tex0, vec2(waterTex1 + disdis*sca2));
nmap = (nmap-ofive) * two;
vec4 vNorm = nmap;
